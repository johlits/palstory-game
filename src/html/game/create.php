            ...
            if ($row_count > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                <tr>
                  ...
                  <td>
                    <?= $row['banned'] == 1 ? "yes" : "no" ?>
                    <button onclick='editSkill(<?= (int)$row["id"] ?>, <?= json_encode($row["skill_id"], JSON_HEX_APOS) ?>, <?= json_encode($row["name"], JSON_HEX_APOS) ?>, <?= json_encode($row["image"], JSON_HEX_APOS) ?>, <?= json_encode($row["description"], JSON_HEX_APOS) ?>, <?= (int)$row["mp_cost"] ?>, <?= (int)$row["cooldown_sec"] ?>, <?= floatval($row["damage_multiplier"]) ?>, <?= (int)$row["unlock_cost"] ?>, <?= json_encode($row["required_job"], JSON_HEX_APOS) ?>, <?= json_encode($row["required_skills"], JSON_HEX_APOS) ?>, <?= json_encode($row["skill_type"], JSON_HEX_APOS) ?>, <?= json_encode($row["stat_modifiers"], JSON_HEX_APOS) ?>, <?= json_encode($row["synergy_with"], JSON_HEX_APOS) ?>, <?= json_encode($row["synergy_bonus"], JSON_HEX_APOS) ?>, <?= (int)$row["synergy_window_sec"] ?>)'>Edit</button>
                    <? if ($row['banned'] == 0 && $_SESSION['secret'] == super_admin_game_hash()) { ?>
